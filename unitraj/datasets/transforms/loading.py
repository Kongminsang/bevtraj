...
class LoadPointsFromMultiSweeps(BaseTransform):
    ...
    def _load_points_feather(self, pts_filename: str) -> np.ndarray:
        """Private function to load point clouds data.
        ...
        Returns:
            np.ndarray: An array containing point clouds data.
        """
        pcd_df = feather.read_feather(pts_filename)
        points = np.array(pcd_df, dtype=np.float32)
        return points
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'lidar_sweeps' not in results:
            ...
            for idx in choices:
                ...
                if sweep['lidar_points']['lidar_path'].endswith(".bin"):
                    points_sweep = self._load_points(
                        sweep['lidar_points']['lidar_path'])
                else:
                    points_sweep = self._load_points_feather(
                        sweep['lidar_points']['lidar_path'])
                points_sweep = np.copy(points_sweep).reshape(-1, self.load_dim)
                if self.remove_close:
                    points_sweep = self._remove_close(points_sweep)
                # bc-breaking: Timestamp has divided 1e6 in pkl infos. (nuscenes)
                # bc-breaking: Timestamp has divided 1e9 in pkl infos. (argo2)
                ...
                elif sweep['lidar_points']['lidar_path'].endswith(".feather"):
                    lidar2ego_s = np.array(sweep['lidar_points']['lidar2ego_s'])
                    points_sweep[:, :
                                3] = points_sweep[:, :3] @ lidar2ego_s[:3, :3]
                    points_sweep[:, :3] -= lidar2ego_s[:3, 3]
                    lidar2ego = np.array(sweep['lidar_points']['lidar2ego'])
                    points_sweep[:, :
                                3] = points_sweep[:, :3] @ lidar2ego[:3, :3]
                    points_sweep[:, :3] -= lidar2ego[:3, 3]
...
class LoadPointsFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if pts_file_path.endswith(".bin"):
            points = self._load_points(pts_file_path)
        elif pts_file_path.endswith(".feather"):
            points = self._load_points_feather(pts_file_path)
            lidar2ego = np.array(results['lidar_points']['lidar2ego'])
            points[:, :
                        3] = points[:, :3] @ lidar2ego[:3, :3]
            points[:, :3] -= lidar2ego[:3, 3]

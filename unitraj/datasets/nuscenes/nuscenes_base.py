...
class BaseDataset(Dataset):
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        if self.serialize_data:
            ...
            #MJTODO: 직렬화를 했기 때문에, bytes를 받으면 이를 pickle.loads했을때 원하는 정보만이 들어오게되는 구조
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        for _ in range(self.max_refetch + 1): #self.max_refetch는 1000
            data = self.prepare_data(idx)
    ...
    def _rand_another(self) -> int:
        ...
        """
        #return np.random.randint(0, len(self)) # idx 복원 추출
        #MODIFY
        return np.random.choice(range(len(self)), size=1, replace=False)

...
class BaseDataset(Dataset):
    ...
    def _join_prefix(self):
        ...
        if not isinstance(self.data_prefix, str):
            raise TypeError('prefix should be a string, but got 'f'{type(self.data_prefix)}')
        if not is_abs(self.data_prefix) and self.data_root:
            self.data_prefix = join_path(self.data_root, self.data_prefix)

...
from .argo2_base import BaseDataset
...
class Det3DDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 data_prefix: str = 'train',
                 data_suffix: dict = dict(pts='sensors/lidar', img=''),
                 ...
                 **kwargs) -> None:
        self.data_suffix = copy.copy(data_suffix)
        ...
        if self.modality['use_lidar']:
            info['lidar_points']['lidar_path'] = \
                osp.join(
                    self.data_prefix,
                    info['scene_token'],
                    self.data_suffix.get('pts', ''),
                    info['lidar_points']['lidar_path'].split(os.sep)[-1])
            ...
            if 'lidar_sweeps' in info:
                for sweep in info['lidar_sweeps']:
                    sweep['lidar_points']['lidar_path'] = osp.join(self.data_prefix,
                                                                   info['scene_token'],
                                                                   self.data_suffix.get('sweeps', ''),
                                                                   sweep['lidar_points']['lidar_path'].split(os.sep)[-1])

        if self.modality['use_camera']:
            for cam_id, img_info in info['images'].items():
                if 'img_path' in img_info:
                    if cam_id in self.data_suffix:
                        cam_suffix = self.data_suffix[cam_id]
                    else:
                        cam_suffix = self.data_suffix.get('img', '')
                    img_info['img_path'] = osp.join(self.data_prefix,
                                                    info['scene_token'],
                                                    cam_suffix,
                                                    img_info['img_path'])

...
class CombinedArgo2(Dataset):
    def __init__(self, config=None, is_validation=False):
        ...
        self.sensor_dataset = Argo2Dataset(**self.sensor_config)
    ...
    def __getitem__(self, idx):
        ...
        sensor_idx = self.traj2sensor(traj_data)
        
        sensor_data = self.sensor_dataset.__getitem__(sensor_idx)
        post_sensor_idx = sensor_data['data_samples'].sample_idx
        
        if sensor_idx != post_sensor_idx:
            post_traj_idx = self._rand_another()
            traj_data, sensor_data = self.__getitem__(post_traj_idx)
        
        #assert
        traj_timestamp = traj_data['curr_timestamp_ns'].item()
        lidar_path = sensor_data['data_samples'].lidar_path
        sensor_timestamp = int(lidar_path.split('/')[-1].split('.')[0])
    ...
    def traj2sensor(self, traj_data):
        scene_token = traj_data['scenario_id'].item()
        curr_timestamp = str(traj_data['curr_timestamp_ns'].item())
        key = scene_token + '.' + curr_timestamp
        sensor_idx = self.token2idx[key]
        return sensor_idx
